...
router.post('/signup', rateLimit({ windowMs: 60000, max: 20, keyPrefix: 'business-signup' }), validateEmailPassword, async (req, res, next) => {
  try {
    ...
    const { data: creatorData, error: creatorError } = await supabaseAdmin
      .from('creators')
      .insert([{ name, role: 'BUSINESS', profile_slug: slug }])
      .select('id');
    ...
    await writeAudit({
      actionType: 'BUSINESS_SIGNUP',
      actor: req.body.email,
      target: `business:${creatorId}`,
    });
...
router.post('/login', rateLimit({ windowMs: 60000, max: 30, keyPrefix: 'business-login' }), validateEmailPassword, async (req, res, next) => {
  try {
    // Query with join
    ...
    const token = generateToken({ id: account.creator_id, role: 'BUSINESS', type: 'business' });
    const refresh = await createRefreshToken({ subjectId: account.creator_id, subjectType: 'BUSINESS' });
...
router.post('/forgot-password/request-code', rateLimit({ windowMs: 60000, max: 10, keyPrefix: 'business-forgot-password-request' }), async (req, res, next) => {
  try {
    ...
    const created = createCode('business', email);
    await sendEmail({
      to: email,
    ...
    await writeAudit({
      actionType: 'BUSINESS_FORGOT_PASSWORD_CODE_SENT',
      actor: email,
      target: `business_account:${account.id}`,
    });
...
router.post('/forgot-password/verify-code', rateLimit({ windowMs: 60000, max: 12, keyPrefix: 'business-forgot-password-verify' }), async (req, res, next) => {
  try {
    const email = normalizeEmail(req.body.email);
    ...
    const verified = verifyCode('business', email, code);
    if (!verified.ok) {
      return res.status(400).json({ error: 'invalid or expired verification code' });
    }
    ...
    // Revoke all tokens for this business account
    await revokeAllForSubject(account.creator_id, 'BUSINESS');
    
    await writeAudit({
      actionType: 'BUSINESS_FORGOT_PASSWORD_RESET',
      actor: email,
      target: `business_account:${account.id}`,
    });
    return res.json({ success: true });
...
router.post('/change-password', verifyToken, async (req, res, next) => {
  try {
    ...
    if (updateError) throw updateError;
    await writeAudit({
      actionType: 'BUSINESS_PASSWORD_CHANGE',
      actor: account.email || `business:${req.user.id}`,
      target: `business:${req.user.id}`,
    });
    return res.json({ success: true });
  } catch (error) {
    return next(error);
...
router.post('/change-email', verifyToken, async (req, res, next) => {
  try {
    ...
    await writeAudit({
      actionType: 'BUSINESS_EMAIL_CHANGE',
      actor: account.email || `business:${req.user.id}`,
      target: `business:${req.user.id}`,
...
router.post('/refresh', rateLimit({ windowMs: 60000, max: 60, keyPrefix: 'business-refresh' }), async (req, res, next) => {
  try {
    ...
    if (tokenRecord.subjectType !== 'BUSINESS') {
    ...
    const accessToken = generateToken({ id: account.creator_id, role: 'BUSINESS', type: 'business' });
    const refreshed = await rotateRefreshToken(raw, { subjectId: tokenRecord.subjectId, subjectType: 'BUSINESS' });
...
router.post('/logout', rateLimit({ windowMs: 60000, max: 60, keyPrefix: 'business-logout' }), async (req, res) => {
  const raw = readRefreshCookie(req);
  if (raw) {
